"""Plot shortest OSM-building-centroid projections to known sewer mains.

Workstream: Codex

This diagnostic assumes gravity mains are known and asks whether building
centroids can explain service laterals as short connections to the nearest main.
That is a service-lateral completion setting, not an anchor-free full-network
prediction setting.
"""
...
def _window_from_summary(summary_path: Path):
    ...
    bounds = summary["window_bounds"]
    return box(*bounds), summary
...
def _centroid_to_main_projections(
    centroids: gpd.GeoDataFrame,
    mains: gpd.GeoDataFrame,
    max_length_m: float,
    truth_laterals: gpd.GeoDataFrame,
) -> gpd.GeoDataFrame:
    if centroids.empty or mains.empty:
        return gpd.GeoDataFrame(geometry=[], crs=centroids.crs or mains.crs)

    main_union = unary_union(list(mains.geometry))
    lateral_coords = None
    lateral_tree = None
    if not truth_laterals.empty:
        lateral_points = truth_laterals.geometry.interpolate(0.5, normalized=True)
        lateral_coords = _coords(gpd.GeoDataFrame(geometry=lateral_points, crs=truth_laterals.crs))
        if len(lateral_coords):
            lateral_tree = cKDTree(lateral_coords)

    rows = []
    for centroid_index, centroid in enumerate(centroids.geometry):
        centroid_point, main_point = nearest_points(centroid, main_union)
        line = LineString([centroid_point, main_point])
        ...
        if lateral_tree is not None and lateral_coords is not None:
            distance, _ = lateral_tree.query([[line.centroid.x, line.centroid.y]], k=1)
            row["nearest_truth_lateral_midpoint_m"] = float(distance[0])
        rows.append(row)
    return gpd.GeoDataFrame(rows, geometry="geometry", crs=centroids.crs or mains.crs)
...
def _plot(
    ...
    bounds_source = mains if not mains.empty else footprints
    ...
    axes[1].set_title("Centroid-to-nearest-main projections")
    ...
    legend_items = [
        ...
        plt.Line2D([0], [0], color="#08519c", lw=2, label="centroid-to-main projection"),
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="#5e3c99", markersize=7, label="OSM building centroid"),
...
def main() -> int:
    ...
    summary_path = source_dir / f"zoom_{int(args.window_size_m)}m" / "summary_zoom.json"
    ...
    centroids = _clip_layer(_read_layer(args.osm_centroids), window, target_crs)
    truth_laterals = _clip_layer(_read_layer(source_dir / "truth_service_laterals.geojson"), window, target_crs)
    mains_zoom = _clip_layer(mains, window, target_crs)

    projections = _centroid_to_main_projections(
        centroids=centroids,
        mains=mains_zoom,
    ...
    output_dir = source_dir / f"zoom_{int(args.window_size_m)}m" / "centroid_to_main"
    output_dir.mkdir(parents=True, exist_ok=True)
    projection_path = output_dir / "osm_centroid_to_main_projections.geojson"
    png_path = output_dir / "osm_centroid_to_main_projections_vs_truth.png"
    summary_path_out = output_dir / "summary_centroid_to_main.json"
    ...
    summary = {
        ...
        "gravity_main_count_in_window": int(len(mains_zoom)),
        "osm_centroid_count_in_window": int(len(centroids)),
    ...
    if not projections.empty:
        ...
        if "nearest_truth_lateral_midpoint_m" in projections:
            summary["median_nearest_truth_lateral_midpoint_m"] = float(
                projections["nearest_truth_lateral_midpoint_m"].median()
            )
    summary_path_out.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    title = (
        f"{args.aoi_id} {args.window_size_m:.0f} m zoom: "
        f"{len(projections):,} centroid-to-main projections"
    )
    _plot(
        truth_laterals=truth_laterals,
        mains=mains_zoom,
