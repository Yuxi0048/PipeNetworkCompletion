...
| `legacy/Inductive.ipynb`, loader cell | Build `LinkNeighborLoader` instances | `scripts/replicate_results.py` |
...
| `legacy/Inductive.ipynb`, training/evaluation cells | Train model variants, save checkpoints, compute metrics | `pipe_network_completion/evaluation.py`, `scripts/replicate_results.py` |
...
| `results/metrics/model_metrics*.csv` | Recorded model metrics from prior runs | Used by `scripts/replicate_results.py --metrics ...` for comparison |
| `models/checkpoints/*.pt` | Saved PyTorch checkpoints | Loaded by `scripts/replicate_results.py --checkpoint ...` |
...
sampling backends, hardware, or random state differ. The replication script sets
...
```powershell
.\.conda\pipe-network-completion\python.exe scripts\replicate_results.py `
